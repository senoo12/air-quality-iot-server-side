...
export default function HistoryPage() {
    const {
        ...
        handleRefresh // 💡 Ambil fungsi handleRefresh dari custom hook
            ...
            <main className="flex-1 p-8 max-w-7xl mx-auto w-full">
                ...
                <header className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-5">
                    ...
                    <div className="flex items-center gap-3 self-start sm:self-center">
                        ...
                        {/* 💡 TOMBOL REFRESH DATA HISTORI MANUAL */}
                        <button
                ...
                {/* MASTER RECORD INGESTION DATA TABLE */}
                <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                    ...
                    <div className="overflow-x-auto">
                        {loading ? (
                            ...
                            <div className="text-center py-16 text-slate-400 text-sm">Tidak ditemukan data history pada perangkat ini.</div>
                        ) : (
                            <table className="w-full text-left border-collapse text-sm">
                                ...
                                <tbody className="divide-y divide-slate-100 text-slate-700">
                                    {sensorHistory.map((row, index) => {
                                        const matchingClass = classHistory[index]?.status || 'Good';
                                        ...
                                        return (
                                            <tr key={row.id} className="hover:bg-slate-50/40 transition-colors">
                                                ...
                                                <td className="p-4">
                                                    <span className={`text-[11px] font-bold px-2.5 py-1 rounded-md border ${getLabelBadgeColor(matchingClass)}`}>
                                                        {matchingClass}
                                                    </span>
                                                </td>
                                                <td className="p-4">
