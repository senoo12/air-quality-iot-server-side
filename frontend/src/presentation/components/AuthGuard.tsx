...
export default function AuthGuard({ children }: { children: React.ReactNode }) {
    ...
    const checkAuth = async () => {
        ...
        // 💡 1. DAFTAR HALAMAN PUBLIK (DAPAT DIAKSES TANPA TOKEN)
        const isPublicPage = pathname === '/login' || pathname === '/register';

        if (isPublicPage) {
            if (token) {
                // Jika sudah punya token aktif, arahkan langsung ke dashboard utama
                router.push('/dashboard');
            } else {
                // Jika belum login, izinkan membuka halaman login atau register
                setAuthorized(true);
            }
            return;
        }
